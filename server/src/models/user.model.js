...
User.getBonusSale = (result) => {
	db.query(
		`SELECT Rlz.bill_1c, Rlz.project_1c, Rlz.doc_num, Rlz.doc_date, Rlz.client_1c, Rlz.client_name, Rlz.doc_sum,
			cuh.email_address HeadManager, cf.MANAGER, cum.email_address Manager, cul.email_address LocalManager,
		...
		LEFT JOIN jiradb.cwd_user cuh ON cuh.user_name=IFNULL(Rlz.HEAD_MANAGER,cf.HEAD_MANAGER)
		LEFT JOIN jiradb.cwd_user cum ON cum.user_name=IFNULL(Rlz.MANAGER,cf.MANAGER)
		LEFT JOIN jiradb.cwd_user cul ON cul.user_name=IFNULL(Rlz.LOC_MAN,
			CASE
			WHEN project_1c IN ('Продажи') THEN IFNULL(cf.OM_SALE,cf.MANAGER)
			WHEN project_1c IN ('Продажи (Склад)', 'Продажи (ЗИП)') THEN IFNULL(cf.OM_SALE_ZIP,cf.MANAGER)
			WHEN project_1c IN ('Ремонт', 'Сервисный контракт', 'Сервисный контракт (ФИКС)', 'Ремонт (ФИКС)', 'Аренда Оборудования', 'СКС') THEN IFNULL(cf.OM_SERV,cf.MANAGER)
			WHEN project_1c IN ('IT') THEN IFNULL(cf.OM_IT,cf.MANAGER)
			END)
...
User.getBonusProfit = (result) => {
	db.query(
		`SELECT Rlz.bill_1c, Rlz.project_1c, Rlz.doc_num, Rlz.doc_date, Rlz.client_1c, Rlz.client_name, Rlz.doc_sum,
			...
			Rlz.Key, Rlz.Status, Rlz.Client, IFNULL(Rlz.Client,CONCAT(Rlz.client_name,' *')) ClientFin, cf.TYPE_TABLE,
			...
			ROUND(IF(Rlz.Key IS NULL,0.2*doc_sum*
			CASE cf.TYPE_TABLE
			WHEN 'client' THEN 0.75
			WHEN 'partner' THEN 0.525
			WHEN 'dealer' THEN 0.3
			ELSE  0.75
			END,
			-1),2) lmpX,
			CONCAT(IF(doc_sum=0,'0 отгрузка',
			IF(ROUND(Rlz.doc_sum/PriceProfit-Rlz.doc_sum*REPLACE(IFNULL(cpm.AIDC_SERV,cpg.AIDC_SERV),',','.'),2)<=0,'0 доход','ок')),IF(Rlz.Client IS NULL,' *','')) Type
		FROM (
			SELECT b.bill_1c, b.project_1c, j.doc_num, j.doc_date, b.client_1c, lp.client_name, j.doc_sum,
...
User.getBonusProfitSum = (email, result) => {
	db.query(
		`SELECT 
			...
			SUM(
			IF(cuh.email_address=${email},
			ROUND(IF(Rlz.Key IS NULL,0.12*doc_sum*
			CASE cf.TYPE_TABLE
			WHEN 'client' THEN 0.75
			WHEN 'partner' THEN 0.525
			WHEN 'dealer' THEN 0.3
			...
			IF(cum.email_address=${email},
			ROUND(IF(Rlz.Key IS NULL,0.05*doc_sum*
			CASE cf.TYPE_TABLE
			WHEN 'client' THEN 0.75
			WHEN 'partner' THEN 0.525
			WHEN 'dealer' THEN 0.3
			...
			IF(cul.email_address=${email},
			ROUND(IF(Rlz.Key IS NULL,0.03*doc_sum*
			CASE cf.TYPE_TABLE
			WHEN 'client' THEN 0.75
			WHEN 'partner' THEN 0.525
			WHEN 'dealer' THEN 0.3
			...
			LEFT JOIN jiradb.customfieldvalue cfvop ON cfvop.ISSUE=ji.ID AND cfvop.CUSTOMFIELD=20822
			LEFT JOIN jiradb.customfieldvalue cfvvi ON cfvvi.ISSUE=ji.ID AND cfvvi.CUSTOMFIELD=21926
			LEFT JOIN tsddb.CLIENTS C
			ON C.ID = ExtractValue(cfvop.TEXTVALUE,'/content/value')
			LEFT JOIN jiradb.jiraissue jid ON jid.ID=il.SOURCE
			LEFT JOIN jiradb.customfieldvalue cfvnd ON cfvnd.ISSUE=jid.ID AND cfvnd.CUSTOMFIELD=21300
			LEFT JOIN jiradb.customfieldvalue cfvp ON cfvp.ISSUE=jid.ID AND cfvp.CUSTOMFIELD=23301
			LEFT JOIN jiradb.customfieldvalue cfvb ON cfvb.ISSUE=jid.ID AND cfvb.CUSTOMFIELD=22704
...
User.getUsersWithBonus = (result) => {
	db.query(
		`SELECT 
			IFNULL(Rlz.HEAD_MANAGER,cf.HEAD_MANAGER) Manager
		FROM (
			SELECT cb.HEAD_MANAGER, cb.MANAGER, project_1c,
			CASE
				WHEN project_1c IN ('Продажи') THEN IFNULL(cb.OM_SALE,cb.MANAGER)
				WHEN project_1c IN ('Продажи (Склад)', 'Продажи (ЗИП)') THEN IFNULL(cb.OM_SALE_ZIP,cb.MANAGER)
				WHEN project_1c IN ('Ремонт', 'Сервисный контракт', 'Сервисный контракт (ФИКС)', 'Ремонт (ФИКС)', 'Аренда Оборудования', 'СКС') THEN IFNULL(cb.OM_SERV,cb.MANAGER)
		...
		UNION
		SELECT 
			IFNULL(Rlz.MANAGER,cf.MANAGER) Manager
		FROM (
			SELECT cb.HEAD_MANAGER, cb.MANAGER, project_1c,
			CASE
				WHEN project_1c IN ('Продажи') THEN IFNULL(cb.OM_SALE,cb.MANAGER)
