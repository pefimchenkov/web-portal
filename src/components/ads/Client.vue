<template>
  <v-container container--fluid>
    ...
    <v-tabs centered icons-and-text v-model="active">
      ...
      <v-tab-item :value="'tab-1'">
        ...
        <div v-else-if="SerialNumbers.length !== 0 && !loadings">
			<v-layout>
			<v-flex xs4>
              <v-text-field
                prepend-icon="search"
        ...
        <div v-else-if="SerialNumbers.length === 0 && !loadings">
          <p class="mt-5" align="center">У данного клиента отсутствуют серийные номера</p>
        </div>
      </v-tab-item>
      ...
      <v-tab-item :value="'tab-2'">
        ...
        <div v-else-if="Contracts.length !== 0 && !loadings">
          <v-data-table
        ...
        <div v-else-if="Contracts.length === 0 && !loadings">
          <p class="mt-5" align="center">У данного клиента отсутствуют договоры</p>
        </div>
      </v-tab-item>
      ...
      <v-tab-item :value="'tab-3'">
        ...
        <div v-else-if="Specprices.length !== 0 && !loadings">
          <v-layout>
            <v-flex xs4>
              <v-text-field
                class="mb-2"
        ...
        <div v-else-if="Specprices.length === 0 && !loadings">
          ...
          <p class="mt-5" align="center">У данного клиента отсутствуют спеццены</p>
        </div>
      </v-tab-item>
    </v-tabs>
  </v-container>
...
export default {
	...
	methods: {
		getClientsList () {
			return Promise.resolve(
				this.ClientList = this.$store.getters.clients
			)
		},
		...
		getContracts (clientId) {
			try {
				this.Contracts = this.$store.getters.contracts.filter(
					a => a.client === parseInt(clientId)
				)
			} catch (error) {
				this.$store.commit('setError', error.message)
		...
		getParentName () {
			let obj = this.ClientList.find(item => parseInt(item.ID) === parseInt(this.id))
			this.ParentName = obj.NAME + ' (' + obj.LEGPERS + ', ' + obj.PROJECT + ')'
		},
		close () {
			this.dialog = false
			this.dialogSN = false
	...
	async created () {
		...
		await this.getContracts(this.id)
		await this.getSpecprices(this.id)
		await this.getZip()
		await this.getCurrency()
		await this.getSerialNumbers(this.id)
