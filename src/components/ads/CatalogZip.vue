					...
					>
						<template v-slot:item="{item}">
							<tr>
								<td class="text-center">{{ item.id }}</td>
								<td class="text-left">{{ item.name }}</td>
								<td class="text-left">{{ item.install_time }}</td>
								<td class="justify-center layout">
									<v-icon small class="mr-2" @click="editItem(item)" :disabled="$acl.not.check('Edit')">edit</v-icon>
									<v-icon small @click="deleteItem(item)" :disabled="$acl.not.check('Edit')">delete</v-icon>
								</td>
...
export default {
	...
	data () {
		return {
			...
			Supplier: [],
			show: false,
			infoText: 'Добавление нового класса',
			search: '',
			localloading: false,
			dialog: false,
			active: 'tab-class',
			...
			headers: [
				...
				{ text: 'Ред.', value: 'edit', align: 'left' }
			],
			headersType: [
				{ text: 'id', value: 'id', align: 'left' },
				{ text: 'Название', value: 'name', align: 'left' },
				{ text: 'Время на установку', value: 'install_time', align: 'celeftnter' },
				{ text: 'Ред.', value: 'edit', align: 'left' }
			],
			pagination: {
				'sortBy': 'id',
