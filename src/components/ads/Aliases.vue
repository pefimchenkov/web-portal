      ...
      <v-data-table
          ...
          >
			...
			<tr>
			  ...
			  <td class="text-right">{{ item.pricends }}
              ...
              <td class="text-left">{{ getDate(item.DATE, 'Specprices') }}</td>
			  </tr>
            </template>
	...
	methods: {
		getSpecprices () {
			try {
				...
				this.Specprices = this.$store.getters.specprices
