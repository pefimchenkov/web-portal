  ...
  <v-container>
    ...
    <div v-if="!loadings && ZipList.length !== 0">
       ...
       <v-flex xs12 md6 offset-sm3>
        <h1 class="text--secondary mb-3">Список ЗИП</h1>
              ...
              <v-layout row>
                ...
                <v-flex xs12>
                  <v-card-title primary-title>
                    <div>
                      <div class="title mb-3">{{ zip.NAME }}</div>
                      <div>Класс: {{ zip.CLASS }}</div>
    ...
    <div v-else="!loadings && ZipList.length === 0">
      <v-layout row>
...
export default {
  ...
  computed: {
    ...
    filteredZip: function () {
      var self = this
      return this.ZipList.data.filter(function (zip) {
        return zip.NAME.toLowerCase().indexOf(self.search.toLowerCase()) !== -1
      })
    },
    loadings () {
