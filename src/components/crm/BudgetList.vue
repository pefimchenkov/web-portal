<template>
		...
		<v-app-bar :fixed="checkBox1" text color="white" class="elevation-2 mb-1">
			...
			<v-flex xs12>
				<v-layout>
					...
					<v-flex xs4 sm2>
						<p class="headline">Текущий реестр</p>
					</v-flex>
					<v-spacer></v-spacer>
      ...
      <div class="data_table_wrapper">
		...
		<template v-slot:body.prepend v-if="mobileView">
			<tr>
				...
				<td v-if="computedHeaders.find(header => header.value === 'name')" colspan="1"></td>
		...
		<template v-slot:item.bill_sum="{ item }">
			{{ item.bill_sum.toLocaleString('ru') }}<v-icon color="primary" small>mdi-currency-rub</v-icon>
		</template>
    ...
    <v-container v-else-if="!loadings && Budget.length === 0" fill-height fluid>
